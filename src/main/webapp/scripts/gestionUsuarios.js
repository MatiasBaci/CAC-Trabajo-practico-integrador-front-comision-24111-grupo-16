...
function cargarUsuarios() {
    ...
    fetch('/CAC-Trabajo-practico-integrador-front-comision-24111-grupo-16/GestionUsuariosServlet')
...
function mostrarModificarModal(id) {
    // fetch(`/proyectoJava_24111/GestionUsuariosServlet?id=${id}`)
    fetch(`/CAC-Trabajo-practico-integrador-front-comision-24111-grupo-16/GestionUsuariosServlet?id=${id}`)
        .then(response => response.json())
        .then(usuario => {
...
function eliminarUsuario(id) {
    ...
    if (confirm('¿Está seguro de que desea eliminar este usuario?')) {
        // fetch(`/proyectoJava_24111/GestionUsuariosServlet?id=${id}`, { method: 'DELETE' })
        fetch(`/CAC-Trabajo-practico-integrador-front-comision-24111-grupo-16/GestionUsuariosServlet?id=${id}`, { method: 'DELETE' })
            .then(response => response.json())
            .then(data => {
                console.log(data.exito)
                if (data.exito) {
