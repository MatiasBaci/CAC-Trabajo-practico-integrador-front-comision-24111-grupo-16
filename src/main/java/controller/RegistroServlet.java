...
//    a los métodos doGet(), doPost() cuando se realiza una solicitud al servlet.
    @Override // TODO: ver si está bien
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        System.out.println("RegistroServlet lanzado");
        ...
        Usuario usuario = new Usuario(nombre, apellido, email, password, Date.valueOf(fechaNacimiento), pais);

        UsuarioDAO usuarioDAO = new UsuarioDAO();
        try {
            usuarioDAO.insertarUsuario(usuario);
            response.sendRedirect("register.html?exito=true");
        } catch (Exception e) {
